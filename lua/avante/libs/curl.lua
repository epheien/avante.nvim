...
util.kv_to_list = function(kv, prefix, sep)
  return compat.flatten(F.kv_map(function(kvp) return { prefix, kvp[1] .. sep .. kvp[2] } end, kv))
end

util.kv_to_str = function(kv, sep, kvsep)
  return F.join(F.kv_map(function(kvp) return kvp[1] .. kvsep .. util.url_encode(kvp[2]) end, kv), sep)
end
...
parse.headers = function(t)
  ...
  local upper = function(str) return string.gsub(" " .. str, "%W%l", string.upper):sub(2) end
...
parse.request = function(opts)
  ...
  if opts.insecure then table.insert(result, "--insecure") end
  if opts.proxy then table.insert(result, { "--proxy", opts.proxy }) end
  if opts.compressed then table.insert(result, "--compressed") end
  ...
  append(opts.raw)
  if opts.output then table.insert(result, { "-o", opts.output }) end
...
local request = function(specs)
  ...
  -- hack for llm stream
  if opts.stream then
    table.insert(args, "--tcp-nodelay") -- 减少延迟
    table.insert(args, "--no-buffer") -- 禁用缓冲
  end
  ...
  if opts.stream then job_opts.on_stdout = opts.stream end
